...
// ==================== SIMULATION ENGINE ====================
// NGBoost-inspired prediction model (simplified deterministic + uncertainty)
const MODEL_PARAMS = {
...
function predictRemoval(adsorbent, pollutant, pH, concentration, time, competition = false) {
  ...
  // Kinetic component (saturating curve)
  const kinetic = 1 - Math.exp(-m.timeK * time);

  // pH effect (Gaussian-like around optimum, direction depends on pollutant)
  let phEffect;
  if (p.phDir === 1) {
...
function bayesianOptimize(pollutant, adsorbent, targetRemoval, phRange, timeRange, concRange, nIter = 60) {
  ...
  for (let i = 0; i < nIter; i++) {
    ...
    // Expected Improvement: balance exploitation (high mean) and exploration (high uncertainty)
...
function MetricCard({ label, value, unit, color = COLORS.primary, subtitle, icon }) {
  return (
    <div style={{
      ...
      border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: "20px 24px",
  ...
  return (
    <div style={{ ...style }}>
      ...
      <select value={value} onChange={e => onChange(e.target.value)} style={{
        width: "100%", padding: "10px 14px", background: COLORS.surface, color: COLORS.text,
        border: `1px solid ${COLORS.border}`, borderRadius: 10, fontSize: 14, outline: "none",
    ...
    <div style={{ display: "flex", gap: 4, background: COLORS.surface, borderRadius: 14, padding: 4, border: `1px solid ${COLORS.border}` }}>
      {tabs.map(t => (
        <button key={t.id} onClick={() => onSelect(t.id)} style={{
          ...
          background: active === t.id ? `linear-gradient(135deg, ${COLORS.primary}20, ${COLORS.secondary}20)` : "transparent",
          color: active === t.id ? COLORS.primary : COLORS.textMuted,
          boxShadow: active === t.id ? `0 0 20px ${COLORS.primary}10` : "none",
        }}>
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 24, minHeight: 600 }}>
      ...
      <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 20, padding: 24, display: "flex", flexDirection: "column", gap: 20 }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: COLORS.text, fontFamily: "'Space Grotesk', sans-serif", paddingBottom: 12, borderBottom: `1px solid ${COLORS.border}` }}>
          ⚙️ Input Parameters
        </div>
        ...
        {/* Prediction Result */}
        <div style={{
          background: `linear-gradient(135deg, ${polColor}10, ${COLORS.surface})`,
          border: `1px solid ${polColor}30`, borderRadius: 16, padding: 20, textAlign: "center", marginTop: "auto",
        }}>
          ...
          <div style={{ fontSize: 48, fontWeight: 800, color: polColor, fontFamily: "'Space Grotesk', sans-serif", lineHeight: 1 }}>
            {pred.mean.toFixed(1)}<span style={{ fontSize: 20 }}>%</span>
          </div>
          <div style={{ fontSize: 12, color: COLORS.textMuted, marginTop: 8 }}>
            95% CI: [{Math.max(0, pred.mean - 1.96 * pred.std).toFixed(1)}%, {Math.min(100, pred.mean + 1.96 * pred.std).toFixed(1)}%]
          </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, flex: 1 }}>
          ...
          <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 20 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.text, marginBottom: 16, fontFamily: "'Space Grotesk', sans-serif" }}>
              ⏱ Adsorption Kinetics — {pol}
            </div>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={shap} layout="vertical" margin={{ left: 80 }}>
                ...
                <Bar dataKey="value" radius={[0, 6, 6, 0]}>
                  {shap.map((entry, i) => (
                    <Cell key={i} fill={entry.value >= 0 ? COLORS.primary : COLORS.accent} fillOpacity={0.8} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 20 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.text, marginBottom: 16, fontFamily: "'Space Grotesk', sans-serif" }}>
              📊 Multi-Pollutant Profile — {ads}
            </div>
            <ResponsiveContainer width="100%" height={220}>
              <RadarChart data={["Cd(II)", "Pb(II)", "As(III)", "Nap"].map(p => ({
                pollutant: p, removal: predictRemoval(ads, p, pH, p === "Nap" ? 50 : conc, time).mean
              }))}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 24, minHeight: 600 }}>
      ...
      <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 20, padding: 24, display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        <button onClick={runOptimization} disabled={running} style={{
          padding: "14px 20px", borderRadius: 12, border: "none", cursor: running ? "wait" : "pointer",
          background: running ? COLORS.border : `linear-gradient(135deg, ${COLORS.primary}, ${COLORS.secondary})`,
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {result ? (
          <>
            ...
            <div style={{
              background: `linear-gradient(135deg, ${polColor}08, ${COLORS.card})`,
              border: `1px solid ${polColor}30`, borderRadius: 20, padding: 24,
            }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: COLORS.text, marginBottom: 16, fontFamily: "'Space Grotesk', sans-serif" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, flex: 1 }}>
              <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 20 }}>
                ...
                <ResponsiveContainer width="100%" height={250}>
                  <ComposedChart data={result.history.map((h, i) => ({
                    ...h, bestSoFar: Math.max(...result.history.slice(0, i + 1).map(x => x.predicted))
                  }))}>
                    <CartesianGrid strokeDasharray="3 3" stroke={COLORS.border} />
                  ...
                  <ScatterChart>
                    ...
                    <Scatter data={result.history} fill={polColor} fillOpacity={0.6}>
                      {result.history.map((entry, i) => (
                        <Cell key={i} fill={entry.predicted >= target ? COLORS.primary : `${COLORS.accent}80`} />
                      ))}
                    </Scatter>
                  </ScatterChart>
                </ResponsiveContainer>
            ...
            <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 20 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.text, marginBottom: 16, fontFamily: "'Space Grotesk', sans-serif" }}>
                🌊 Sensitivity Analysis — Operating Window
              </div>
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={(() => {
                  const data = [];
                  for (let p = phMin; p <= phMax; p += 0.2) {
                    const pred = predictRemoval(ads, pol, p, result.best.concentration, result.best.time);
                    data.push({ pH: +p.toFixed(1), removal: pred.mean, upper: Math.min(100, pred.mean + 1.96 * pred.std), lower: Math.max(0, pred.mean - 1.96 * pred.std) });
                  }
                  return data;
                })()}>
                  <defs>
          ...
          <div style={{
            ...
            background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 20, padding: 40,
          }}>
            ...
            <div style={{ fontSize: 13, color: COLORS.textMuted, marginTop: 8, textAlign: "center", maxWidth: 400 }}>
              Set your remediation objectives and parameter search bounds, then launch Bayesian optimization to find the optimal process conditions.
            </div>
          </div>
        )}
      </div>
...
function ModelBenchmarkPanel() {
  ...
  // Generate predicted vs actual scatter data for top model
  const scatterData = useMemo(() => {
    const data = [];
    for (let i = 0; i < 60; i++) {
      const actual = 10 + Math.random() * 85;
      const noise = (Math.random() - 0.5) * 8;
      data.push({ actual: +actual.toFixed(1), predicted: +(actual + noise).toFixed(1) });
    }
    return data;
  }, []);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.text, marginBottom: 16, fontFamily: "'Space Grotesk', sans-serif" }}>
            🎯 NGBoost: Predicted vs. Actual (R² = 0.982)
          </div>
          <ResponsiveContainer width="100%" height={320}>
            <ScatterChart>
              <CartesianGrid strokeDasharray="3 3" stroke={COLORS.border} />
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, fontFamily: "'Space Grotesk', sans-serif" }}>
          <thead>
            <tr style={{ borderBottom: `2px solid ${COLORS.border}` }}>
              {["Rank", "Model", "Category", "R²", "RMSE", "Performance"].map(h => (
                <th key={h} style={{ padding: "10px 16px", textAlign: "left", color: COLORS.textMuted, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {models.map((m, i) => (
              <tr key={m.name} style={{ borderBottom: `1px solid ${COLORS.border}`, transition: "background 0.2s" }}>
                <td style={{ padding: "12px 16px", color: COLORS.textDim }}>#{i + 1}</td>
                <td style={{ padding: "12px 16px", color: COLORS.text, fontWeight: 600 }}>
                  <GlowDot color={m.color} /> <span style={{ marginLeft: 8 }}>{m.name}</span>
                  {i === 0 && <span style={{ marginLeft: 8, fontSize: 10, background: `${COLORS.primary}20`, color: COLORS.primary, padding: "2px 8px", borderRadius: 20 }}>BEST</span>}
                </td>
                ...
                <td style={{ padding: "12px 16px" }}>
                  <div style={{ height: 6, background: COLORS.surface, borderRadius: 3, overflow: "hidden", width: 120 }}>
                    <div style={{ height: "100%", width: `${m.r2 * 100}%`, background: m.color, borderRadius: 3, transition: "width 0.5s" }} />
                  </div>
                </td>
              </tr>
            ))}
...
function ValidationPanel() {
  ...
  const barData = validationData.map(v => ({
    name: `${v.scenario}\n${v.target}`,
    Predicted: v.predicted,
    Experimental: v.experimental,
    error: v.expStd,
    absError: Math.abs(v.predicted - v.experimental).toFixed(1),
  }));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.text, marginBottom: 16, fontFamily: "'Space Grotesk', sans-serif" }}>
            📊 Predicted vs. Experimental Removal
          </div>
      ...
      <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 20 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.text, marginBottom: 16, fontFamily: "'Space Grotesk', sans-serif" }}>
          📋 Independent Experimental Validation Results
        </div>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, fontFamily: "'Space Grotesk', sans-serif" }}>
          <thead>
            <tr style={{ borderBottom: `2px solid ${COLORS.border}` }}>
              {["Scenario", "Pollutant", "Conditions", "Predicted (%)", "Experimental (%)", "|Error|"].map(h => (
                <th key={h} style={{ padding: "10px 14px", textAlign: "left", color: COLORS.textMuted, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.06em" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
              ...
              <tr key={i} style={{ borderBottom: `1px solid ${COLORS.border}` }}>
                ...
                <td style={{ padding: "12px 14px" }}>
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                    <GlowDot color={POLLUTANT_COLORS[v.target]} size={6} /> 
                    <span style={{ color: POLLUTANT_COLORS[v.target] }}>{v.target}</span>
                  </span>
                </td>
                ...
                <td style={{ padding: "12px 14px" }}>
                  <span style={{
                    background: `${COLORS.primary}15`, color: COLORS.primary, padding: "3px 10px",
                    borderRadius: 20, fontSize: 12, fontWeight: 600,
                  }}>
                    {Math.abs(v.predicted - v.experimental).toFixed(1)}%
                  </span>
                </td>
              </tr>
            ))}
...
function MaterialComparisonPanel() {
  ...
  const heatmapData = useMemo(() => {
    return adsorbents.flatMap(a => pollutants.map(p => ({
  ...
  const cycleData = useMemo(() => {
    return Array.from({ length: 10 }, (_, i) => ({
      cycle: i + 1,
      "Cd(II)": Math.max(75, 98 - i * 1.8 + (Math.random() - 0.5) * 2).toFixed(1),
      "Pb(II)": Math.max(78, 99 - i * 1.5 + (Math.random() - 0.5) * 2).toFixed(1),
      "As(III)": Math.max(60, 90 - i * 2.5 + (Math.random() - 0.5) * 2).toFixed(1),
      "Nap": Math.max(55, 85 - i * 2.0 + (Math.random() - 0.5) * 3).toFixed(1),
    }));
  }, []);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 24 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.text, marginBottom: 20, fontFamily: "'Space Grotesk', sans-serif" }}>
          🔥 Removal Efficiency Heatmap (pH=6, t=30 min)
        </div>
        <div style={{ display: "grid", gridTemplateColumns: `100px repeat(${pollutants.length}, 1fr)`, gap: 4 }}>
          <div />
          {pollutants.map(p => (
            <div key={p} style={{ textAlign: "center", fontSize: 12, fontWeight: 600, color: POLLUTANT_COLORS[p], padding: "8px 0" }}>{p}</div>
          ))}
          {adsorbents.map(a => (
            <>
              ...
              {pollutants.map(p => {
                ...
                const bg = val > 90 ? `${COLORS.primary}${Math.round(intensity * 60 + 20).toString(16)}`
                  : val > 70 ? `${COLORS.secondary}${Math.round(intensity * 50 + 20).toString(16)}`
                  : val > 50 ? `${COLORS.warning}${Math.round(intensity * 40 + 20).toString(16)}`
                  : `${COLORS.accent}${Math.round(intensity * 30 + 20).toString(16)}`;
                return (
                  <div key={`${a}-${p}`} style={{
                    background: bg, borderRadius: 10, padding: "16px 8px", textAlign: "center",
                    border: `1px solid ${COLORS.border}`, transition: "transform 0.2s",
                  }}>
                    <div style={{ fontSize: 20, fontWeight: 700, color: COLORS.text }}>{val.toFixed(0)}%</div>
                    <div style={{ fontSize: 10, color: COLORS.textMuted, marginTop: 2 }}>Removal</div>
                  </div>
                );
              })}
            </>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.text, marginBottom: 16, fontFamily: "'Space Grotesk', sans-serif" }}>
            ♻️ MNCJG Cycling Stability (10 Cycles)
          </div>
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={cycleData}>
              ...
              <Legend wrapperStyle={{ fontSize: 11 }} />
              {Object.entries(POLLUTANT_COLORS).map(([name, color]) => (
                <Line key={name} type="monotone" dataKey={name} stroke={color} strokeWidth={2} dot={{ r: 3, fill: color }} />
              ))}
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.text, marginBottom: 16, fontFamily: "'Space Grotesk', sans-serif" }}>
            ⚡ Kinetic Performance vs. Literature
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, fontFamily: "'Space Grotesk', sans-serif" }}>
            <thead>
              <tr style={{ borderBottom: `2px solid ${COLORS.border}` }}>
                {["Material", "Pollutant", "Time", "Removal"].map(h => (
                  <th key={h} style={{ padding: "8px 10px", textAlign: "left", color: COLORS.textMuted, fontSize: 10, textTransform: "uppercase" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        select:focus { border-color: ${COLORS.primary}; box-shadow: 0 0 0 2px ${COLORS.primaryDim}; }
        tr:hover { background: ${COLORS.surfaceHover} !important; }
      `}</style>

      {/* Header */}
      <header style={{
        padding: "20px 32px", borderBottom: `1px solid ${COLORS.border}`,
        background: `linear-gradient(180deg, ${COLORS.surface}dd, ${COLORS.bg})`,
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 11, color: COLORS.textMuted, marginTop: 2, fontFamily: "'JetBrains Mono', monospace" }}>
                  ML-Driven Coconut Shell Biochar/MnOx Composite Design for Heavy Metal–Naphthalene Co-contamination
                </p>
              </div>
            </div>
          </div>
      ...
      <main style={{ maxWidth: 1400, margin: "0 auto", padding: "24px 32px" }}>
        ...
        <div style={{ marginTop: 24 }}>
          {activeTab === "predict" && (
            <>
              <SectionTitle subtitle="NGBoost probabilistic prediction engine with SHAP-based explainability. Adjust parameters to explore the multi-dimensional adsorption response landscape.">
          ...
          {activeTab === "optimize" && (
            <>
              <SectionTitle subtitle="Bayesian optimization with Expected Improvement acquisition function. Define remediation targets and let the algorithm discover optimal process conditions.">
          ...
          {activeTab === "benchmark" && (
            <>
              <SectionTitle subtitle="Systematic comparison of 9 ML algorithms spanning linear, instance-based, bagging, and boosting paradigms on independent test set.">
          ...
          {activeTab === "validate" && (
            <>
              <SectionTitle subtitle="Three independent validation scenarios (VS1–VS3) with conditions absent from the training database. Paired t-test confirms no significant prediction bias.">
          ...
          {activeTab === "material" && (
            <>
              <SectionTitle subtitle="Comparative analysis across five adsorbent variants (CSC→NC→MNC→NCJG→MNCJG) and cycling stability assessment over 10 regeneration cycles.">
      ...
      <footer style={{ padding: "20px 32px", borderTop: `1px solid ${COLORS.border}`, marginTop: 40 }}>
        <div style={{ maxWidth: 1400, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontSize: 11, color: COLORS.textMuted }}>
            Wang X.Y. et al. · Henan University · Engineering Research Center for Nanomaterials
          </div>
          <div style={{ fontSize: 10, color: COLORS.textMuted, fontFamily: "'JetBrains Mono', monospace" }}>
            Built with NGBoost · SHAP · Bayesian Optimization · n=285 experimental datapoints
          </div>
        </div>
      </footer>
    </div>
